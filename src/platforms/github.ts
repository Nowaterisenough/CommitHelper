 ...
 * GitHub 平台实现
 */
export class GitHubPlatform implements IPlatform {
    ...
    constructor(httpClient: HttpClient, logger: Logger) {
        ...
        this.logger = logger;
        this.tokenCache = new Cache<string>(60); // 60分钟缓存
    }
    ...
    async fetchIssues(repoInfo: RepoInfo, accessToken: string, maxIssues: number): Promise<Issue[]> {
        ...
        while (allIssues.length < maxIssues) {
            const url = `https://api.github.com/repos/${repoInfo.owner}/${repoInfo.repo}/issues?state=open&page=${page}&per_page=${perPage}`;
            const headers = {
